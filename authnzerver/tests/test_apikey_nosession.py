...
permissions_json = os.path.join(this_dir,
                                '..',
...
def test_issue_apikey(tmpdir):
    ...
    assert (
        apikey["ipa"] == apikey_issue_payload["ip_address"]
    )
    assert (
        apikey["uid"] == apikey_issue_payload["user_id"]
    )
    assert (
        apikey["rol"] == apikey_issue_payload["user_role"]
    )
...
def test_verify_apikey(tmpdir):
    ...
    payload = {'full_name': 'Another Test User',
