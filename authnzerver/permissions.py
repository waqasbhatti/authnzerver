...
def pii_sha256_hash(item, salt):
    ...
    return sha256(("%s%s" % (item, salt)).encode("utf-8")).hexdigest()[:24]
...
def pii_blake2b_hash(item, key):
    ...
    hasher = blake2b(
        key=key.encode("utf-8"), digest_size=12, person=b"authnzrv-pii"
    )
...
def load_permissions_json(model_json: str) -> dict:
...
def get_item_actions(
    ...
    try:
        target_valid_actions = item_policy[target_name]["valid_role_actions"]
        target_valid_visibilities = item_policy[target_name][
            "valid_visibilities"
        ]
        target_invalid_roles = item_policy[target_name]["invalid_roles"]

        if debug:
            print("%s valid_perms: %r" % (target_name, target_valid_actions))
            print(
                "%s valid_visibilities: %r"
                % (target_name, target_valid_visibilities)
            )
            print("%s invalid_roles: %r" % (target_name, target_invalid_roles))
        ...
        # 'for_other'), check ownership actions for target status
            ...
            role_actions = role_policy[role_name]["allowed_actions_for_other"][
                target_visibility
            ]

        # these are the final available actions
        available_actions = role_actions.intersection(target_valid_actions)
...
def check_item_access(
    ...
    role: str = "anonymous",
    action: str = "view",
    target_name: str = "collection",
    target_owner: int = 1,
    target_visibility: str = "private",
    target_sharedwith: str = None,
    debug: bool = False,
) -> bool:
    """
    ...
    if debug:
        print(
            "target: '%s' may be owned by role: '%s' = %s"
...
def check_role_limits(
    ...
    role: str,
    limit_name: str,
    value_to_check: Union[float, int],
) -> bool:
    """
    ...
    try:
        ...
        LOGGER.error(
            "Could not convert value to be checked "
...
def load_policy_and_check_limits(
    ...
    return check_role_limits(
        permissions_model, role, limit_name, value_to_check
    )
