...
def send_email(
    ...
    if "<" in sender and ">" in sender:
        ...
        if not sender_email:
            LOGGER.error(
                "Invalid sender email address. Can't send this email."
            )
            return False
        else:
    ...
    # validate them, and then add them to the validated_recipients list
        ...
        for bcc_recipient in bcc:
            ...
            if "<" in cleaned_bcc_recipient and ">" in cleaned_bcc_recipient:
                bcc_recipient_email = re.findall(
                    r"<(\S+)>", cleaned_bcc_recipient
                )
    ...
    try:
        ...
        if server.has_extn("STARTTLS"):
            ...
            try:
                ...
                server.sendmail(
                    cleaned_sender, validated_recipients, msg.as_string()
                )
            ...
            LOGGER.error(
                "Email server: %s does not support STARTTLS, "
...
def send_signup_verification_email(
    ...
    if payload["created_info"]["send_verification"] is False:
        ...
        LOGGER.error(
            ...
            % (
                payload["reqid"],
                pii_hash(
                    payload["created_info"]["user_id"], payload["pii_salt"]
                ),
    ...
    user_sel = (
        ...
        .where(users.c.user_id == payload["created_info"]["user_id"])
    )
    ...
    if email_sent:
        ...
        upd = (
            users.update()
            .where(users.c.user_id == payload["created_info"]["user_id"])
            .where(users.c.is_active.is_(False))
            .where(users.c.email == payload["created_info"]["user_email"])
            .values(
                {
...
def set_user_emailaddr_verified(
    ...
    sel = (
        select(users.c.user_id, users.c.user_role, users.c.extra_info)
        .select_from(users)
...
def send_forgotpass_verification_email(
    ...
    user_sel = (
        ...
        .where(users.c.user_role != "locked")
        .where(users.c.user_role != "anonymous")
    )
